...
def getCountOfFingers():
    ...
    cv2.createTrackbar('trh1', 'trackbar', threshold, 100, printThreshold) #creating trackbar
    ...
    while camera.isOpened():
     ...
     #  Capturing the background and processing
        if isBgCaptured == 1:  # this part wont run until background captured
            ...
            cv2.imshow('blur', blur) #showing the blured image
            ret, thresh = cv2.threshold(blur, threshold, 255, cv2.THRESH_BINARY)
            cv2.imshow('ori', thresh) #showing the threshold image
